...
static bool g_adc_dma_hw_started = false;
...
static inline void filter_reset(bool clear_published)
{
    g_ring_idx = 0;
    g_blocks_filled = 0;
    g_scale = kScale32;
...
static inline void process_half_update_filter(const uint32_t* p_half)
{
    ...
    g_last_progress_tick = time_ticks32();
    g_adc_dma_inited = true;
...
static void adc_dma_restart_transfer()
{
    ...
    g_last_restart_tick = time_ticks32();
    g_last_progress_tick = g_last_restart_tick;
}
...
void ADC_DMA_poll()
{
    ...
    if (g_adc_dma_hw_started)
    {
        const uint32_t now = time_ticks32();
        const uint32_t stalled = ms_to_ticks32(250u);
        if ((uint32_t)(now - g_last_progress_tick) > stalled &&
            (uint32_t)(now - g_last_restart_tick) > stalled)
            adc_dma_restart_transfer();
    }
}
...
bool ADC_DMA_sample_ready()
{
    return (g_sample_ready != 0u);
}
...
void ADC_DMA_init()
{
    ...
    filter_reset(true);
    g_last_progress_tick = time_ticks32();
    g_last_restart_tick = g_last_progress_tick;
    ...
    dma.DMA_MemoryBaseAddr     = (uint32_t)(uintptr_t)g_dma_buf;
