...
static void filament_rgb(uint8_t R, uint8_t G, uint8_t B, uint8_t cap,
                         uint8_t *rr, uint8_t *gg, uint8_t *bb)
{
    *rr = scale8_video(kGamma8[R], cap);
    *gg = scale8_video(scale8_video(kGamma8[G], 176u), cap);
    *bb = scale8_video(scale8_video(kGamma8[B], 240u), cap);
}
#endif
...
void WS2812_class::clear(void)
{
    ...
    for (uint32_t i = 0; i < (uint32_t)num; i++) last_online_filament_cap[i] = 0u;
    dirty = true;
}
...
void WS2812_class::write_frame(uint8_t override_index, uint32_t override_grb)
{
    ...
    for (uint32_t led = 0; led < (uint32_t)num; led++)
    {
        ...
        uint32_t v = led == override_index ? override_grb : last_grb[led];
...
void WS2812_class::preview_RGB(uint8_t R, uint8_t G, uint8_t B, uint8_t index)
{
    if (index >= num) return;
    const uint32_t packed = ((uint32_t)G << 16) | ((uint32_t)R << 8) | (uint32_t)B;
    write_frame(index, packed);
}

void WS2812_class::preview_filament_RGB(uint8_t R, uint8_t G, uint8_t B, uint8_t index, uint8_t cap)
{
    if (index >= num) return;
#if BMCU_ONLINE_LED_FILAMENT_RGB
    uint8_t rr, gg, bb;
    filament_rgb(R, G, B, cap, &rr, &gg, &bb);
    preview_RGB(rr, gg, bb, index);
#else
    (void)cap;
    preview_RGB(R, G, B, index);
#endif
}

void WS2812_class::restore(void)
{
    write_frame(0xFFu, 0u);
    dirty = false;
}
...
void WS2812_class::set_RGB_online(uint8_t R, uint8_t G, uint8_t B, uint8_t index, bool filament)
{
    ...
    const uint8_t cap = lighting_filament_cap();
    if (last_online_is_filament[index] &&
            (last_online_raw_rgb[index] == raw) &&
            (last_online_filament_cap[index] == cap))
        return;
